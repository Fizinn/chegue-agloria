...
get().atualizarStatus({ pronto, fase: pronto ? "aguardando_copa" : "montando_time" });
  ...
  revelarCopa: async () => {
    ...
    // Sorteia também a ORDEM da chave — sem isso, os confrontos seguiam a
    // ordem de entrada na sala e os mesmos jogadores quase sempre caíam um
    // contra o outro na mesma fase, toda vez. Embaralha (Fisher-Yates) uma
    // única vez aqui; depois disso a chave fica fixa (quem ganha continua
    // enfrentando quem ganhar do outro lado, normalmente).
    const idsEmbaralhados =
      sala.config.chaveamento ?? (() => {
    ...
    const novaConfig = { ...sala.config, anoCopa, chaveamento: idsEmbaralhados };
