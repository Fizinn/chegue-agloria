...
const [verMeuTime, setVerMeuTime] = useState(false);
  ...
  useEffect(() => {
    if (sala?.status !== "draft" || draft.timeCompleto) return;
  ...
  }, [sala?.status, draft.timeCompleto, tempoTotalSegundos]);

  if (!sala || !meuId) return null;

  if (fase === "draft") {
    if (draft.timeCompleto && sala.status !== "em_andamento") {
  ...
  // A ordem dos participantes no banco define a posição fixa na chave.
  // `fase` aqui é o estado do lobby/draft, não a rodada do mata-mata, então
  // não podemos filtrar por `fase === "torneio"`.
  const indiceChave = Math.max(0, participantes.findIndex((p) => p.id === meuId));
  ...
  return (
    <Torneio
      ...
      rivaisMultiplayer={rivaisMultiplayer as { id: string; apelido: string; forcaTime: number; rodadaAtual: number; indiceChave: number; eliminado: boolean; vencedorNome?: string; vencedorForca?: number; vencedorJogadores?: JogadorReal[]; vencedorSigla?: string; vencedorAno?: number }[]}
