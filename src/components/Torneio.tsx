...
import { Evento, PartidaAoVivo } from "@/components/PartidaAoVivo";
...
interface Adversario { nome: string; forca: number; real: boolean; jogadores?: JogadorReal[]; sigla?: string; ano?: number; }
...
export function Torneio({
  ...
  const adversario = useMemo<Adversario>(() => {
    ...
    if (!solo) {
      ...
      if (vencedorReal) {
        return { nome: vencedorReal.apelido, forca: vencedorReal.forcaTime, real: true };
      }
