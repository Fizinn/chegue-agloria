...
// Resultado pré-calculado por src/lib/simulacaoDeterministica.ts, usado
// quando o adversário é OUTRO JOGADOR REAL. "mine"/"rival" já está na
// perspectiva de quem está vendo a tela (eu sempre apareço como "casa"),
// então esse componente só precisa "reproduzir" o roteiro pronto em vez de
// sortear os próprios eventos — garantindo que os dois jogadores, cada um
// no seu navegador, cheguem exatamente no mesmo placar e vencedor.
export interface EventoResultadoDeterministico {
  minuto: number;
  tipo: "gol-mine" | "gol-rival" | "amarelo-mine" | "vermelho-mine" | "amarelo-rival" | "vermelho-rival";
  slotIndice: number;
}
...
export interface ResultadoDeterministico {
  meusGols: number;
  golsRival: number;
  eventos: EventoResultadoDeterministico[];
  penaltis?: {
    meusGols: number;
    golsRival: number;
    cobrancas: { lado: "mine" | "rival"; converteu: boolean; slotIndice: number }[];
  };
}
...
interface PartidaAoVivoProps {
  ...
  /** Presente só em partidas jogador-vs-jogador — quando fornecido, o
...
// Traduz o "slotIndice" (0-10, anônimo) do roteiro determinístico pra um
// jogador de verdade do MEU time, na hora de nomear quem fez o gol/levou o
// cartão. Isso é só cosmético — o placar e o vencedor já vêm prontos do
// roteiro, então essa escolha não pode divergir entre os dois navegadores
// nem afetar o resultado, só o texto exibido.
function jogadorPorSlotIndice(
  indice: number,
  titulares: Record<string, JogadorReal | null>,
  slots: SlotFormacao[],
  expulsos: Set<string>,
  ...
  const lista = slots
    ...
    .filter((j): j is JogadorReal => j != null && !expulsos.has(j.id));
...
export function PartidaAoVivo({
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      // Partida contra outro jogador real: o placar e os eventos já vêm
      // prontos (mesma seed nos dois navegadores), então aqui só
      // "reproduzimos" o minuto — nunca sorteamos nada de novo. É isso que
      // garante que os dois lados chegam exatamente no mesmo resultado.
      if (resultadoDeterministico) {
        const eventosDoMinuto = resultadoDeterministico.eventos.filter((e) => e.minuto === novoMinuto);
        eventosDoMinuto.forEach((evento) => {
          if (evento.tipo === "gol-mine") {
            const artilheiro = jogadorPorSlotIndice(evento.slotIndice, titulares, slots, expulsosRef.current);
          ...
          } else if (evento.tipo === "amarelo-mine") {
            const alvo = jogadorPorSlotIndice(evento.slotIndice, titulares, slots, expulsosRef.current);
            if (alvo) {
              const atual = (amarelosRef.current[alvo.id] ?? 0) + 1;
              amarelosRef.current = { ...amarelosRef.current, [alvo.id]: atual };
              registrarEvento({ minuto: novoMinuto, texto: `🟨 Cartão amarelo pra ${alvo.nome}.`, tipo: "amarelo" });
            }
          } else if (evento.tipo === "vermelho-mine") {
            const alvo = jogadorPorSlotIndice(evento.slotIndice, titulares, slots, expulsosRef.current);
            if (alvo) {
          ...
          } else if (evento.tipo === "amarelo-rival") {
            registrarEvento({ minuto: novoMinuto, texto: `🟨 Cartão amarelo pro ${nomeFora}.`, tipo: "amarelo" });
          } else if (evento.tipo === "vermelho-rival") {
            registrarEvento({ minuto: novoMinuto, texto: `🟥 Vermelho pro ${nomeFora}!`, tipo: "vermelho" });
          }
        });
        return;
      }
  ...
  function baterPenaltis() {
    if (resultadoDeterministico?.penaltis) {
      // Mesma lógica da fase normal: quem bate e a ordem são só cosméticos
      // (o jogador escolhe pra dar nome às cobranças), mas se CONVERTE ou
      // não já vem pronto do roteiro determinístico — assim os dois
      // navegadores sempre fecham a disputa de pênaltis com o mesmo placar.
      const { penaltis } = resultadoDeterministico;
      let proximoBatedorCasa = 0;
      const cobrancas: CobrancaPenalti[] = penaltis.cobrancas.map((c) => {
        if (c.lado === "mine") {
          const batedor = ordemPenaltis.length > 0 ? ordemPenaltis[proximoBatedorCasa % ordemPenaltis.length] : undefined;
          proximoBatedorCasa++;
          return { lado: "casa", jogador: batedor?.nome, converteu: c.converteu };
        }
        return { lado: "fora", converteu: c.converteu };
      });
      setResultadoPenaltis({ golsCasa: penaltis.meusGols, golsFora: penaltis.golsRival });
      setCobrancasPenaltis(cobrancas);
      setFase("penaltis");
      return;
    }
